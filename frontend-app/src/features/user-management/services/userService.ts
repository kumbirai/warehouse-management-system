...
export const userService = {
  async listUsers(filters: UserListFilters): Promise<UserListApiResponse> {
    ...
    // Send tenantId as X-Tenant-Id header (backend expects it as header, not query param)
    if (filters.tenantId) {
  ...
  async createUser(request: CreateUserRequest): Promise<ApiResponse<CreateUserResponse>> {
    const response = await apiClient.post<ApiResponse<CreateUserResponse>>(USER_BASE_PATH, request);
    return response.data;
  },
  ...
  async activateUser(userId: string): Promise<ApiResponse<void>> {
    const response = await apiClient.put<ApiResponse<void>>(`${USER_BASE_PATH}/${userId}/activate`);
    return response.data;
  },
  ...
  async suspendUser(userId: string): Promise<ApiResponse<void>> {
    const response = await apiClient.put<ApiResponse<void>>(`${USER_BASE_PATH}/${userId}/suspend`);
    return response.data;
  },
