...
import { ConsignmentCsvError, UploadConsignmentCsvResponse } from '../types/stockManagement';
import CloudUploadIcon from '@mui/icons-material/CloudUpload';

interface ConsignmentCsvUploadFormProps {
  onUpload: (file: File) => Promise<UploadConsignmentCsvResponse>;
  isLoading: boolean;
}

export const ConsignmentCsvUploadForm = ({
  onUpload,
  isLoading,
}: ConsignmentCsvUploadFormProps) => {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [uploadResult, setUploadResult] = useState<UploadConsignmentCsvResponse | null>(null);
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Upload a CSV file containing stock consignment data. Maximum file size: 10MB.
      </Typography>
      ...
      <Box sx={{ mb: 3 }}>
        <input
          ...
          style={{ display: 'none' }}
          id="consignment-csv-file-input"
        />
        <label htmlFor="consignment-csv-file-input">
          <Button
            variant="outlined"
            component="span"
          ...
          {uploadResult.errors && uploadResult.errors.length > 0 && (
            <TableContainer component={Paper} sx={{ mt: 2 }}>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {uploadResult.errors.map((error: ConsignmentCsvError, index: number) => (
                    <TableRow key={index}>
                      <TableCell>{error.rowNumber}</TableCell>
                      <TableCell>{error.consignmentReference || '-'}</TableCell>
