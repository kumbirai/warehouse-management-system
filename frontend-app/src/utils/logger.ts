...
const formatMessage = (level: LogLevel, message: string, context?: Record<string, unknown>): string => {
    ...
    const contextStr = enhancedContext && Object.keys(enhancedContext).length > 0
 ...
 */
const sendToRemote = (level: LogLevel, message: string, context?: Record<string, unknown>): void => {
