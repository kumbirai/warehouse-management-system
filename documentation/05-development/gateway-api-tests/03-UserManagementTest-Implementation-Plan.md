...
## Overview
`UserManagementTest` validates all user management functionality through the gateway service. Tests authenticate as both SYSTEM_ADMIN and TENANT_ADMIN to verify user CRUD operations, role assignment, user lifecycle management (activate, deactivate, suspend), and tenant-scoped access control.
