...
`AuthenticationTest` validates the complete authentication flow through the gateway BFF (Backend-For-Frontend) endpoints. Tests verify login, token refresh, logout, JWT token
handling, httpOnly cookie management, token expiration, and security controls that mirror the frontend application's authentication behavior.
