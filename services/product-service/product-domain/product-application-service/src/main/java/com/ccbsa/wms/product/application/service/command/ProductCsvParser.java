 ...
 * - Validate CSV format
 * - Extract product data from CSV rows
...
public class ProductCsvParser {
     ...
     * - Required columns: product_code, description, primary_barcode, unit_of_measure
     * - Optional columns: secondary_barcode, category, brand
     *
    ...
    private void validateHeaders(Set<String> headers) {
        ...
        if (!missingHeaders.isEmpty()) {
            throw new IllegalArgumentException(
                    String.format("Missing required CSV headers: %s", String.join(", ", missingHeaders))
            );
        }
    }
