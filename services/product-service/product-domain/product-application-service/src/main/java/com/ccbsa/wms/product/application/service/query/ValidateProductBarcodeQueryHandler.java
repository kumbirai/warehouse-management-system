    ...
    public ValidateProductBarcodeResult handle(ValidateProductBarcodeQuery query) {
        ...
        try {
            ...
            return ValidateProductBarcodeResult.builder()
                    .valid(false)
                    .barcodeFormat(BarcodeType.CODE_128) // Default for invalid format
                    .errorMessage(String.format("Invalid barcode format: %s", e.getMessage()))
                    .build();
        }
        ...
        if (product.isEmpty()) {
            return ValidateProductBarcodeResult.builder()
                    .valid(false)
                    .barcodeFormat(barcodeFormat)
                    .errorMessage(String.format("Product with barcode '%s' not found", query.getBarcode()))
                    .build();
        }
