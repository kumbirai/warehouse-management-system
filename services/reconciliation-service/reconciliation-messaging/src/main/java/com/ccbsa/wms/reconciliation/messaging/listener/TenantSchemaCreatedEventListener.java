 ...
 * <p>
 * This listener implements schema-per-tenant pattern by: 1. Creating the tenant schema in reconciliation database 2. Running Flyway migrations in the tenant schema to create
 * tables and indexes
 * <p>
...
public class TenantSchemaCreatedEventListener {
    ...
    @KafkaListener(topics = "tenant-events", groupId = "reconciliation-service", containerFactory = "externalEventKafkaListenerContainerFactory")
    ...
    private void runFlywayMigrations(String schemaName) {
        ...
        try {
            ...
            if (migrationsApplied > 0) {
                ...
                log.debug("No Flyway migrations found for reconciliation-service (expected - this service may not have migrations): schemaName={}", schemaName);
            }
        } catch (Exception e) {
