 ...
 * <p>
 * Eureka stores service names in uppercase (e.g., TENANT-SERVICE), but services
 * use lowercase names in URLs (e.g., http://tenant-service). This configuration
 * ensures that service lookups work by trying both the original service name and
 * its uppercase variant when querying Eureka.
 * <p>
 * This configuration applies to all LoadBalancer clients in user-service via the
...
public class LoadBalancerConfig {
    ...
    static class CaseInsensitiveServiceInstanceListSupplierDecorator implements ServiceInstanceListSupplier {
        ...
        public Flux<List<ServiceInstance>> get() {
            ...
            // CRITICAL: Eureka stores service names in uppercase (e.g., TENANT-SERVICE),
            // but services use lowercase in URLs (e.g., http://tenant-service).
