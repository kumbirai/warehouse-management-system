 ...
 * <p>
 * Listens to TenantSchemaCreatedEvent and creates tenant schema with tables in user database.
 * <p>
 * This listener implements schema-per-tenant pattern by: 1. Creating the tenant schema in user database 2. Running Flyway migrations in the tenant schema to create tables and
 * indexes
 * <p>
...
public class TenantSchemaCreatedEventListener {
    ...
    @KafkaListener(topics = "tenant-events", groupId = "user-service", containerFactory = "externalEventKafkaListenerContainerFactory")
    public void handle(@Payload Map<String, Object> eventData, @Header(value = "__TypeId__", required = false) String eventType,
        ...
        try {
            ...
            try {
                ...
                log.debug("Could not extract tenantId from event data", ex);
            }
            try {
                ...
                log.debug("Could not extract schemaName from event data", ex);
            }

            log.error("Error processing TenantSchemaCreatedEvent: eventId={}, tenantId={}, schemaName={}, error={}", eventId, tenantId, schemaName, e.getMessage(), e);
