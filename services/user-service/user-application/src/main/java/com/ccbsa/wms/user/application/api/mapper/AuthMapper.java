 ...
 * Mapper: AuthMapper
 * <p>
 * Maps between API DTOs and application service DTOs.
 * Anti-corruption layer between API and application service layers.
 */
@Component
public class AuthMapper {
    public LoginCommand toLoginCommand(LoginRequest request) {
        return new LoginCommand(request.getUsername(),
                request.getPassword());
    }
    ...
    public LoginResponse toLoginResponse(AuthenticationResult result) {
        ...
        LoginResponse.UserContext apiContext = new LoginResponse.UserContext(context.getUserId()
                .getValue(),
                context.getUsername(),
                context.getTenantId() != null ? context.getTenantId()
                        .getValue() : null,
                context.getRoles(),
                context.getEmail(),
    ...
    public UserContextResponse toUserContextResponse(UserContextView view) {
        ...
        return new UserContextResponse(view.getUserId()
                .getValue(),
                view.getUsername(),
                view.getTenantId() != null ? view.getTenantId()
                        .getValue() : null,
                view.getRoles(),
                view.getEmail(),
                view.getFirstName(),
