 ...
 * <p>
 * Listens to TenantSchemaCreatedEvent and creates tenant schema with tables in picking database.
 * <p>
 * This listener implements schema-per-tenant pattern by: 1. Creating the tenant schema in picking database 2. Running Flyway migrations in the tenant schema to create tables and
 * indexes
 * <p>
...
public class TenantSchemaCreatedEventListener {
    ...
    @KafkaListener(topics = "tenant-events", groupId = "picking-service", containerFactory = "externalEventKafkaListenerContainerFactory")
    ...
    private void runFlywayMigrations(String schemaName) {
        ...
        try {
            ...
            if (migrationsApplied > 0) {
                ...
                logger.debug("No Flyway migrations found for picking-service (expected - this service may not have migrations): schemaName={}", schemaName);
            }
        } catch (Exception e) {
