...
public class GetPickingListQueryHandler {
    ...
    public PickingListQueryResult handle(GetPickingListQuery query) {
        ...
        return PickingListQueryResult.builder()
                .id(pickingList.getId())
                .pickingListReference(pickingList.getPickingListReference())
                .status(pickingList.getStatus())
                .receivedAt(pickingList.getReceivedAt())
                .processedAt(pickingList.getProcessedAt())
                .loadCount(pickingList.getLoadCount())
                .totalOrderCount(pickingList.getTotalOrderCount())
                .notes(pickingList.getNotes() != null ? pickingList.getNotes().getValue() : null)
                .loads(new java.util.ArrayList<>(loadResults))
                .build();
    }
    ...
    private PickingListQueryResult.LoadQueryResult toLoadQueryResult(Load load, List<Order> orders, TenantId tenantId) {
        ...
        return PickingListQueryResult.LoadQueryResult.builder()
                .loadId(load.getId().getValueAsString())
                .loadNumber(load.getLoadNumber().getValue())
                .status(load.getStatus() != null ? load.getStatus().name() : null)
                .orderCount(load.getOrderCount())
    ...
    private PickingListQueryResult.OrderQueryResult toOrderQueryResult(Order order, TenantId tenantId) {
        ...
        return PickingListQueryResult.OrderQueryResult.builder()
