...
public class KeycloakGroupAdapter
    ...
    private GroupRepresentation findGroup(String realmName, String groupName) {
        RealmResource realmResource = realm(realmName);
        try {
            GroupRepresentation representation = realmResource.getGroupByPath(String.format("/%s", groupName));
    ...
    private GroupRepresentation createGroup(String realmName, TenantId tenantId, String groupName) {
        ...
        attributes.put(TENANT_ID_ATTRIBUTE, List.of(tenantId.getValue()));
        attributes.put(STATUS_ATTRIBUTE, List.of("ACTIVE"));
        group.setAttributes(attributes);

        try (Response response = groupsResource.add(group)) {
            int status = response.getStatus();
            if (status >= 400) {
                throw new IllegalStateException(String.format("Failed to create Keycloak group %s in realm %s. Status: %d", groupName, realmName, status));
            }
        }

        return findGroup(realmName, groupName);
    }

    private void updateStatusAttribute(String realmName, GroupRepresentation group, String status) {
        Map<String, List<String>> attributes = group.getAttributes() != null ? new HashMap<>(group.getAttributes()) : new HashMap<>();
