 ...
 * <p>
 * Implements KeycloakRealmServicePort for tenant-service.
 * Provides Keycloak realm management operations.
 */
@Component
public class KeycloakRealmAdapter implements KeycloakRealmServicePort {
    ...
    @Override
    public RealmRepresentation createRealm(TenantId tenantId,
                                           String realmName) {
        logger.info("Creating Keycloak realm: {} for tenant: {}",
                realmName,
                tenantId);
        ...
        if (existingRealm != null) {
            logger.info("Keycloak realm already exists: {}. Enabling if disabled.",
                    realmName);
        ...
        realm.setDisplayName(String.format("Tenant: %s",
                tenantId.getValue()));
        ...
        try {
            ...
            logger.info("Keycloak realm created: {}",
                    realmName);
            ...
            if (e.getResponse() != null && e.getResponse().getStatus() == Response.Status.CONFLICT.getStatusCode()) {
                logger.warn("Keycloak realm already exists (409 Conflict): {}. This may be due to a race condition. Enabling realm.",
                        realmName);
    ...
    public RealmRepresentation getRealm(String realmName) {
        try {
            ...
            logger.debug("Error checking realm existence: {}",
                    realmName,
                    e);
            return null;
        } catch (Exception e) {
    ...
    public void enableRealm(String realmName) {
        logger.info("Enabling Keycloak realm: {}",
                realmName);

        RealmRepresentation realm = getRealm(realmName);
        if (realm == null) {
            throw new IllegalArgumentException(String.format("Realm not found: %s",
                    realmName));
        }
        ...
        logger.info("Keycloak realm enabled: {}",
                realmName);
    }

    @Override
    public void updateRealm(String realmName,
                            RealmRepresentation realm) {
        logger.info("Updating Keycloak realm: {}",
                realmName);
        ...
        logger.info("Keycloak realm updated: {}",
                realmName);
    }

    @Override
    public void disableRealm(String realmName) {
        logger.info("Disabling Keycloak realm: {}",
                realmName);
        ...
        logger.info("Keycloak realm disabled: {}",
                realmName);
    }
