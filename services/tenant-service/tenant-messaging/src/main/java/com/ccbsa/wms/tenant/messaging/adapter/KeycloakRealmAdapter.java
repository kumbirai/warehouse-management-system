...
public class KeycloakRealmAdapter implements KeycloakRealmServicePort {
    ...
    public RealmRepresentation createRealm(TenantId tenantId, String realmName) {
        log.info("Creating Keycloak realm: {} for tenant: {}", realmName, tenantId);

        // Check if realm already exists first
        RealmRepresentation existingRealm = getRealm(realmName);
        if (existingRealm != null) {
            log.info("Keycloak realm already exists: {}. Enabling if disabled.", realmName);
        ...
        try {
            ...
            if (e.getResponse() != null && e.getResponse().getStatus() == Response.Status.CONFLICT.getStatusCode()) {
                log.warn("Keycloak realm already exists (409 Conflict): {}. This may be due to a race condition. Enabling realm.", realmName);
    ...
    public RealmRepresentation getRealm(String realmName) {
        try {
            ...
            if (e.getResponse() != null && e.getResponse().getStatus() == Response.Status.NOT_FOUND.getStatusCode()) {
                log.debug("Realm not found: {}", realmName);
                return null;
            }
            // Log other client errors but still return null
            log.debug("Error checking realm existence: {}", realmName, e);
            return null;
        } catch (Exception e) {
            log.debug("Realm not found: {}", realmName, e);
            return null;
        }
    }
    ...
    public void updateRealm(String realmName, RealmRepresentation realm) {
        log.info("Updating Keycloak realm: {}", realmName);

        RealmResource realmResource = keycloakClient.getAdminClient().realms().realm(realmName);
        realmResource.update(realm);

        log.info("Keycloak realm updated: {}", realmName);
    }
