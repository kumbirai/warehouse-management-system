 ...
 * <p>
 * Listens to TenantSchemaCreatedEvent and creates tenant schema with tables in integration database.
 * <p>
 * This listener implements schema-per-tenant pattern by: 1. Creating the tenant schema in integration database 2. Running Flyway migrations in the tenant schema to create tables
 * and indexes
 * <p>
    ...
    private void runFlywayMigrations(String schemaName) {
        ...
        try {
            ...
            if (migrationsApplied > 0) {
                ...
                logger.debug("No Flyway migrations found for integration-service (expected - this service may not have migrations): schemaName={}", schemaName);
            }
        } catch (Exception e) {
