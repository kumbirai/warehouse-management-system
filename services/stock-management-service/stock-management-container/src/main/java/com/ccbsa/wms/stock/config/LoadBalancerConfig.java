 ...
 * <p>
 * Eureka stores service names in uppercase (e.g., PRODUCT-SERVICE, LOCATION-MANAGEMENT-SERVICE, USER-SERVICE), but services
 * use lowercase names in URLs (e.g., http://product-service, http://location-management-service, http://user-service). This configuration
 * ensures that service lookups work by trying both the original service name and
 * its uppercase variant when querying Eureka.
 * <p>
 * This configuration applies to all LoadBalancer clients in stock-management-service via the
 * default configuration.
 * <p>
 * <b>CRITICAL:</b> For RestTemplate with @LoadBalanced, the service ID is extracted
 * from the URL hostname (e.g., "product-service" from "http://product-service/api/...").
...
public class LoadBalancerConfig {
    ...
    static class CaseInsensitiveServiceInstanceListSupplierConfiguration {
        ...
        public ServiceInstanceListSupplier caseInsensitiveServiceInstanceListSupplier(ConfigurableApplicationContext context, DiscoveryClient discoveryClient) {
            ...
            // (e.g., "product-service" from "http://product-service/api/...")
    ...
    static class CaseInsensitiveServiceInstanceListSupplierDecorator implements ServiceInstanceListSupplier {
        ...
        public Flux<List<ServiceInstance>> get() {
            ...
            // CRITICAL: Eureka stores service names in uppercase (e.g., PRODUCT-SERVICE, LOCATION-MANAGEMENT-SERVICE, USER-SERVICE),
            // but services use lowercase in URLs (e.g., http://product-service, http://location-management-service, http://user-service).
