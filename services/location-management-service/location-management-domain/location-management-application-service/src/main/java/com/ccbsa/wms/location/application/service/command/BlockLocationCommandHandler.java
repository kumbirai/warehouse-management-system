...
/**
 * Command Handler: BlockLocationCommandHandler
 * <p>
 * Handles blocking of a Location aggregate.
 * <p>
 * Responsibilities:
 * - Loads Location aggregate
 * - Executes business logic via aggregate (block method)
...
public class BlockLocationCommandHandler {
    ...
    public BlockLocationResult handle(BlockLocationCommand command) {
        log.debug("Handling BlockLocationCommand for location: {}", command.getLocationId());
        ...
        // 7. Return result
        return BlockLocationResult.builder().locationId(savedLocation.getId()).status(savedLocation.getStatus()).lastModifiedAt(savedLocation.getLastModifiedAt()).build();
    }
     ...
     */
    private void validateCommand(BlockLocationCommand command) {
        if (command == null) {
        ...
        if (command.getBlockedBy() == null) {
            throw new IllegalArgumentException("BlockedBy is required");
        }
