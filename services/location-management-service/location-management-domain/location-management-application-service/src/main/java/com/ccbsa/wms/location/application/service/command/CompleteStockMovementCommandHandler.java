...
/**
 * Command Handler: CompleteStockMovementCommandHandler
 * <p>
 * Handles completion of a StockMovement aggregate.
 * <p>
 * Responsibilities:
 * - Loads StockMovement aggregate
 * - Executes business logic via aggregate (complete method)
...
public class CompleteStockMovementCommandHandler {
    ...
    public CompleteStockMovementResult handle(CompleteStockMovementCommand command) {
        log.debug("Handling CompleteStockMovementCommand for movement: {}", command.getStockMovementId());
        ...
        // 7. Return result
        return CompleteStockMovementResult.builder().stockMovementId(savedMovement.getId()).status(savedMovement.getStatus()).completedAt(savedMovement.getCompletedAt()).build();
    }
     ...
     */
    private void validateCommand(CompleteStockMovementCommand command) {
        if (command == null) {
        ...
        if (command.getCompletedBy() == null) {
            throw new IllegalArgumentException("CompletedBy is required");
        }
    }
