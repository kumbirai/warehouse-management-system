...
public class ListLocationsQueryHandler {
    ...
    public ListLocationsQueryResult handle(ListLocationsQuery query) {
        ...
        // API uses 0-based pagination (consistent with Spring Data JPA and frontend)
        ...
        List<LocationView> locationViews = viewRepository.findByTenantIdWithFilters(query.getTenantId(), query.getZone(), query.getStatus(), query.getSearch(), page, size);
        ...
        // 5. Build result
        // Return 0-based page in response (consistent with API contract and frontend which uses 0-based)
        int responsePage = query.getPage() != null && query.getPage() >= 0 ? query.getPage() : 0;
    ...
    @SuppressFBWarnings(value = "UPM_UNCALLED_PRIVATE_METHOD", justification = "Method is called via method reference in stream (this::toLocationQueryResult) - SpotBugs false "
            + "positive")
    private LocationQueryResult toLocationQueryResult(LocationView view) {
