 ...
 * <p>
 * Eureka stores service names in uppercase (e.g., TENANT-SERVICE, USER-SERVICE), but services
 * use lowercase names in URLs (e.g., http://tenant-service, http://user-service). This configuration
 * ensures that service lookups work by trying both the original service name and
 * its uppercase variant when querying Eureka.
 * <p>
 * This configuration applies to all LoadBalancer clients in notification-service via the
 * default configuration.
 * <p>
 * <b>CRITICAL:</b> For RestTemplate with @LoadBalanced, the service ID is extracted
 * from the URL hostname (e.g., "tenant-service" from "http://tenant-service/api/...").
...
public class LoadBalancerConfig {
    ...
    static class CaseInsensitiveServiceInstanceListSupplierConfiguration {
        ...
        public ServiceInstanceListSupplier caseInsensitiveServiceInstanceListSupplier(ConfigurableApplicationContext context, DiscoveryClient discoveryClient) {
            ...
            // (e.g., "tenant-service" from "http://tenant-service/api/...")
    ...
    static class CaseInsensitiveServiceInstanceListSupplierDecorator implements ServiceInstanceListSupplier {
        ...
        public Flux<List<ServiceInstance>> get() {
            ...
            // CRITICAL: Eureka stores service names in uppercase (e.g., TENANT-SERVICE, USER-SERVICE),
            // but services use lowercase in URLs (e.g., http://tenant-service, http://user-service).
