...
public class UserRoleAssignedEventListener {
    private static final Logger logger = LoggerFactory.getLogger(UserRoleAssignedEventListener.class);

    private final CreateNotificationCommandHandler createNotificationCommandHandler;

    public UserRoleAssignedEventListener(CreateNotificationCommandHandler createNotificationCommandHandler) {
        ...
        try {
            ...
            try {
                ...
                CreateNotificationCommand command = CreateNotificationCommand.builder().tenantId(tenantId).recipientUserId(UserId.of(aggregateId)).recipientEmail(recipientEmail)
                        .title(Title.of("Role Assigned")).message(Message.of(String.format("The role '%s' has been assigned to your account.", roleName)))
                        .type(NotificationType.USER_ROLE_ASSIGNED).build();
