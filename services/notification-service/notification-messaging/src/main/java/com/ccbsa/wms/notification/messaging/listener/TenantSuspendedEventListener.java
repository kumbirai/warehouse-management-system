 ...
 * <p>
 * Listens to TenantSuspendedEvent and creates tenant suspension notification.
 * <p>
...
public class TenantSuspendedEventListener {
    ...
    public void handle(@Payload Map<String, Object> eventData, @Header(value = "__TypeId__", required = false) String eventType,
        ...
        try {
            ...
            if (!isTenantSuspendedEvent(detectedEventType)) {
                log.debug("Skipping event - not TenantSuspendedEvent: detectedType={}, headerType={}", detectedEventType, eventType);
            ...
            log.info("Received TenantSuspendedEvent: tenantId={}, eventId={}, eventDataKeys={}", tenantId.getValue(), extractEventId(eventData), eventData.keySet());
            ...
            try {
                ...
                // Create tenant suspension notification
                ...
                CreateNotificationCommand command =
                        CreateNotificationCommand.builder().tenantId(tenantId).recipientUserId(UserId.of(UUID.fromString("00000000-0000-0000-0000-000000000000")))
                                .recipientEmail(tenantEmail).title(Title.of("Tenant Suspended"))
                                .message(Message.of("Your tenant account has been suspended. Please contact your administrator for more information."))
                                .type(NotificationType.TENANT_SUSPENDED).build();

                createNotificationCommandHandler.handle(command);

                log.info("Created tenant suspension notification for tenant: tenantId={}, email={}", tenantId.getValue(), tenantEmail.getValue());
            ...
            log.error("Invalid event format for TenantSuspendedEvent: eventData={}, error={}", eventData, e.getMessage(), e);
            acknowledgment.acknowledge();
        } catch (Exception e) {
            log.error("Failed to process TenantSuspendedEvent: eventData={}, error={}", eventData, e.getMessage(), e);
    ...
    private String detectEventType(Map<String, Object> eventData, String headerType) {
        ...
        if ("Tenant".equals(aggregateType)) {
            ...
            // Without @class or header, default to TenantSuspendedEvent for this listener
            // The isTenantSuspendedEvent check will determine if we should process it
            log.debug("No @class or header found, defaulting to TenantSuspendedEvent for aggregateType=Tenant");
    ...
    /**
     * Checks if the event is a TenantSuspendedEvent.
     *
     * @param eventType Event type to check
     * @return true if this is a TenantSuspendedEvent
     */
    private boolean isTenantSuspendedEvent(String eventType) {
        return eventType != null && eventType.contains("TenantSuspendedEvent");
    }
