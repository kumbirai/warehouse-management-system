...
public class UserRoleRemovedEventListener {
    ...
    @KafkaListener(topics = "user-events", groupId = "notification-service-user-role-removed", containerFactory = "externalEventKafkaListenerContainerFactory")
    public void handle(@Payload Map<String, Object> eventData, @Header(value = "__TypeId__", required = false) String eventType,
        ...
        try {
            ...
            if (!isUserRoleRemovedEvent(detectedEventType)) {
                log.debug("Skipping event - not UserRoleRemovedEvent: detectedType={}, headerType={}", detectedEventType, eventType);
            ...
            // Log event receipt
            log.info("Received UserRoleRemovedEvent: userId={}, tenantId={}, role={}, eventId={}, eventDataKeys={}", aggregateId, tenantId.getValue(), roleName,
            ...
            try {
                ...
                // Create role removal notification (email is optional - handler supports null email)
                CreateNotificationCommand.Builder commandBuilder =
                        CreateNotificationCommand.builder().tenantId(tenantId).recipientUserId(UserId.of(aggregateId)).title(Title.of("Role Removed"))
                                .message(Message.of(String.format("The role '%s' has been removed from your account.", roleName))).type(NotificationType.USER_ROLE_REMOVED);
                ...
                log.info("Created role removal notification for user: userId={}, email={}, role={}", aggregateId, emailStr, roleName);
            ...
            log.error("Invalid event format for UserRoleRemovedEvent: eventData={}, error={}", eventData, e.getMessage(), e);
            acknowledgment.acknowledge();
        } catch (Exception e) {
            log.error("Failed to process UserRoleRemovedEvent: eventData={}, error={}", eventData, e.getMessage(), e);
    ...
    private String detectEventType(Map<String, Object> eventData, String headerType) {
        ...
        // Check for UserRoleRemovedEvent-specific fields (has roleName)
        if (eventData.containsKey("roleName")) {
            // Could be UserRoleAssignedEvent or UserRoleRemovedEvent
            // Default to UserRoleRemovedEvent for this listener
