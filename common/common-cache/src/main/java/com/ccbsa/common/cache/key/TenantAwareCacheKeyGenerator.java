...
public class TenantAwareCacheKeyGenerator implements KeyGenerator {
    ...
    public Object generate(Object target, Method method, Object... params) {
        ...
        keyBuilder.append(TENANT_PREFIX).append(KEY_SEPARATOR).append(tenantId).append(KEY_SEPARATOR);

        // 3. Append method parameters
        if (params != null && params.length > 0) {
            String paramsKey = Arrays.stream(params).map(this::extractKeyValue).collect(Collectors.joining(KEY_SEPARATOR));
