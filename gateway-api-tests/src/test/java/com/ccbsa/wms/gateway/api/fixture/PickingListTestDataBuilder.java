...
public class PickingListTestDataBuilder {
    ...
    public static CreatePickingListRequest buildCreatePickingListRequest() {
        return CreatePickingListRequest.builder().loads(List.of(buildLoadRequest())).notes("Test picking list").build();
    }
    ...
    public static LoadRequest buildLoadRequest(String loadNumber) {
        return LoadRequest.builder().loadNumber(loadNumber).orders(List.of(buildOrderRequest())).build();
    }
    ...
    public static OrderRequest buildOrderRequest(String orderNumber) {
        return OrderRequest.builder().orderNumber(orderNumber).customerCode("CUST-" + TestData.faker().code().ean8()).customerName(TestData.faker().company().name())
                .priority("HIGH").lineItems(List.of(buildOrderLineItemRequest())).build();
    }
    ...
    public static OrderLineItemRequest buildOrderLineItemRequest(String productCode, int quantity) {
        return OrderLineItemRequest.builder().productCode(productCode).quantity(quantity).notes("Test line item").build();
    }
    ...
    public static CreatePickingListRequest buildCreatePickingListRequestWithMultipleOrders() {
        ...
        return CreatePickingListRequest.builder().loads(loads).notes("Test picking list with multiple loads").build();
    }

    public static CreatePickingListRequest buildInvalidPickingListRequest() {
        // Missing required fields
        return CreatePickingListRequest.builder().loads(List.of()).build();
    }
}
